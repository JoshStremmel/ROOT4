...
─────────────────────────
League-agnostic team strength scoring from parsed game data. Used as a
tiebreaker in game-importance ranking so that even when two games have the
same base importance, the one involving stronger teams scores higher.
...
Five weighted signals (all normalized to [0, 1] over active teams):
  ...
  3. Group (e.g. division) standing bonus  weight 0.20  (already [0,1])
...
Signal 3 needs a grouping concept (NFL: division) supplied by the caller as
`group_key_fn` — a team-abbreviation → group-key function. Any league with
some notion of a standings sub-group (division, conference, pool, ...) can
reuse this unchanged by supplying its own grouping function; a league with
no such concept can pass `lambda abbr: ""` and signal 3 becomes a no-op tie
(all teams share the empty group).
"""
...
def compute_all_team_strengths(
    ...
    group_key_fn : team abbr → group key (e.g. division), used for signal 3
    ...
    dict: teamAbbr → {
        ...
        "groupBonus":           float,   # signal 3 (already [0,1])
    ...
    # Signal 3: group (e.g. division) standing bonus
    _compute_group_bonus(raw, all_games, standings_map, group_key_fn)
...
def print_strength_table(
    ...
    header = (
        ...
        f"  {'PtDiff':>7}  {'SOS':>7}  {'GroupRnk':>7}  {'Form':>7}  {'Margin':>7}"
    )
    print(header)
    print("  " + "-" * 91)
    for abbr, data in rows:
        ...
        print(
            ...
            f"  {data['groupBonus']:>7.4f}  {data['recentForm']:>7.4f}"
...
def _compute_group_bonus(
    raw: dict[str, dict],
    all_games: list[dict],
    standings_map: dict[str, dict],
    group_key_fn: Callable[[str], str],
) -> None:
    """Assign s3_raw (group standing bonus) to each team in raw."""
    ...
    # Group teams by the caller's grouping key (e.g. division)
    groups: dict[str, list[str]] = {}
    for abbr in raw:
        key = group_key_fn(abbr)
        if key:
            groups.setdefault(key, []).append(abbr)
