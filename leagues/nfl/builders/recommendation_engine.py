...
from engine.rdf_utils import safe_bool, safe_float, safe_int
    ...
    def _fetch_relevant_games(self) -> list[dict[str, Any]]:
        ...
        for row in self.dataset.query(q):
            rows.append({
                ...
                "home_moneyline": safe_int(row.homeMoneyLine),
                "away_moneyline": safe_int(row.awayMoneyLine),
                "home_favorite" : safe_bool(row.homeFavorite),
            })
        return rows
